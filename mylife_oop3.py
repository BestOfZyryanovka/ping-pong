...
class Board:
    ...
    def brandom(self):
        ...
        self.slab.config(text='Generation {}s. There are {}s creatures'.format(self.loop.get(),
                                                                         self.alive.get()))
    ...
    def figures(self):
        ...
        #        self.root.update()
        key = random.choice(list(figdic.keys()))
        coors = figdic[key]
        t = 0
        for pos in coors:
            self.cells[pos][-1] = True
    ...
    def run(self):
        ...
        while game:
            ...
            if (int(self.alive.get()) == 0):
                msg = 'All creatures are dead after {} steps!'.format(-1)
